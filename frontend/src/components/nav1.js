    ...
    <>
      ...
      >
        {other.children ? (
          ...
          >
            <div className="flex width-100 justify-between align-center">
              <section
                id="nav-left"
      ...
      <div
        className="respond-max"
        style={{
          minWidth: vertical ? SECTIONS.NAV.WIDTH : "",
          minHeight: vertical ? "" : THEME.NAV.HEIGHT,
...
function fillNav(items) {
  ...
  return items.map((item, i) => {
    ...
    return (
      <Link
        href={href}
        style={{ color: THEME.SECONDARY, ...style }}
        {...other}
        key={`nav-${i}`}
        className={`${className}`}
      >
