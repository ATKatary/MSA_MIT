    ...
    return (
        <div id={props.id} className="width-100 height-75vw margin-20px flex justify-center">
            {images.map((image, index) => {
              return <Button key={index} className={`${image['color']} project-box box-shadow cycle-start${index}-animation padding-0 pointer flex column opacity-95`} onClick={() => {window.location = image['link']}}>
